...
from composer_request import make_iap_request as iap


def trigger(dag_id):

    """This function calls the make_iap_request function which is defined
     at
    ...
    """
    url = 'https://e1aa6ac3476a1effa-tp.appspot.com/api/experimental/dags/%s/dag_runs' % dag_id
    client_id = '552275855417-ibhgc253s3kgdsr9pivo013dhfp9kh5v.apps.googleusercontent.com'
    ...
    conf = {
        ...
        'transformed_path': 'dummy_transformed_path',
        'failed_path': 'dummy_failed_path',
    }

    # The api signature requires a unique run_id
    payload = {
        'run_id': 'triggered-run-%s' % datetime.now(_LOCAL_TZ).strftime('%Y%m%d%H%M%S%Z'),
        #'run_id': 'post-triggered-run-111',
        'conf': json.dumps(conf),
    }

    return iap.make_iap_request(url, client_id, method='POST',
                                data=json.dumps(payload))


if __name__ == "__main__":
    trigger('cloud_genesis_hk')
